...
/* Azure CDN */
/* Error :
    cdn.ProfilesClient#Create: Failure sending request: StatusCode=400 -- Original Error: Code="BadRequest" Message="CDN profiles cannot be created with a Student account."
...
resource "azurerm_cdn_endpoint" "cdng5storagefile" {
  name                = "storagefilecdn"
  profile_name        = azurerm_cdn_profile.cdng5.name
  location            = azurerm_resource_group.rg.location
  resource_group_name = azurerm_resource_group.rg.name
  ...
  origin {
    name      = "storagefile"
    host_name = "https://filestorageg5esgi.blob.core.windows.net/filestoragecontainerg5esgi"
  }
}
*/

/* Azure Storage */
resource "azurerm_storage_account" "sgfile" {
  name                     = var.storage_account_name_file
  resource_group_name      = azurerm_resource_group.rg.name
  location                 = azurerm_resource_group.rg.location
  account_tier             = "Standard"
  account_replication_type = "LRS"
...
/*Storage container*/
resource "azurerm_storage_container" "sgcontainerfile" {
  name                  = var.storage_container_name_file
  storage_account_name  = azurerm_storage_account.sgfile.name
  container_access_type = "private"
}
...
resource "azurerm_storage_blob" "storagefile" {
  name                   = "Sujet.pdf"
  storage_account_name   = azurerm_storage_account.sgfile.name
  storage_container_name = azurerm_storage_container.sgcontainerfile.name
  type                   = "Block"
  source                 = "Sujet.pdf"
}